...
one. [ADR 0015](adr/0015-staged-filesystem-generation.md) records the
decision this document keeps current.
...
result into place, cleaning up a failed one, and the terminal messages for
each.
...
system temp directory — writes every rendered file into it, and finalises
with `Path.rename`. Same-volume placement is what makes that rename an
...
| Engine (`pipeline.finalise_generation_request`) | The staging directory is renamed to `dst`; no intermediate state is ever visible at `dst`. | The staging directory is removed; `dst` is left exactly as it was found — created or not. |
...
`--dry-run` short-circuits before staging is attempted on either path: the
...
A rejected non-empty destination and a staging or finalisation failure both
...
- [`tests/test_staging.py`](../tests/test_staging.py) — destination conflict
  ...
  temp directory); atomic finalisation; staging-tree cleanup on failure,
  including read-only files; `discard_on_failure` removing only a
  destination it created.
- [`tests/test_pipeline.py`](../tests/test_pipeline.py) —
  `finalise_generation_request` against a real `RenderedProject`, and a
  failure mid-write leaving nothing behind.
- [`tests/test_cli.py`](../tests/test_cli.py) — `--engine-preview` against a
  non-empty destination exits before the engine is touched;
  `--engine-preview --dry-run` writes nothing; the existing
