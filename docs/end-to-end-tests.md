...
reuses the same candidate wheel and the published `forge-template 0.6.0` to
prove CF-22.01's containment and CF-22.02's recovery through the installed
console rather than through `create_forge.update`'s own functions. One project
is generated by the console and copied per scenario, beside an outside directory
holding a sentinel file no update may touch. The suite covers the normal route
(a no-op update, a committed local edit preserved, `--dry-run`); the degraded
route with a real changed, removed and added target staged by the real `git add
-A`; `--dry-run` on both routes leaving the tree, index and status
byte-identical; containment refusals (a recorded target outside the project, a
recorded Git-hook target, an invalid late target beside a valid changed one, a
symlinked parent), each with the sentinel unchanged and nothing changed; and
both Git recovery states — a completed staged update and an interrupted one —
recovered by the commands the console *prints*, pasted from outside the
repository, plus a dirty tree refused with no command offered.

The console cannot produce a changed update offline on its own (with the
recorded provider equal to the installed one every target classifies
`unchanged`) and a subprocess offers no failure injection, so the suite edits
the recorded per-target digests in the committed `.forge/generation.json` to
make `update --degraded` see a pristine, changed, removed and added target, and
uses a pre-existing `.git/index.lock` to make the real `git add -A` fail. It
runs on Linux in the `e2e` job and on Windows in `e2e-windows`, which names it
