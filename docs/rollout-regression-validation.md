...
`tests/test_e2e_installed_rollout.py` builds one `create-forge` candidate
wheel (shared with CF-14.02 through `tests/conftest.py`'s `candidate_wheel`
fixture) and installs it three ways. Since ADR 0040 (CF-18.01) made
`forge-template` a required dependency and demoted `copier` to the optional
`legacy` extra, a plain wheel install always resolves a compatible engine now
— `engineless_client` and `out_of_range_client` each force a *broken* state
afterward rather than reaching one through a normal install:

| Environment | Install | What it proves |
| --- | --- | --- |
| `installed_client` | `wheel[legacy]` + `forge-template 0.5.0` | Library and CLI Application default engine paths; the `--legacy` Copier route on the same install; the failure matrix; the atomic lock failure |
| `engineless_client` | `wheel[legacy]`, then `forge-template` uninstalled | the `--legacy` Copier path with no engine installed; `--version`, `list --legacy`, `doctor --json`, and the default `new` path's exit-`3` rejection with no engine installed |
| `out_of_range_client` | `wheel`, then a real `forge-template 0.3.2` (below the supported range) forced in | the exit-`3` compatibility boundary; `doctor` reporting the out-of-range package |
...
| Commands that do not use the engine remain usable without the engine installed | `test_engineless_version_is_the_candidate`, `test_engineless_list_shows_the_bundled_registry` (the wheel's `templates.toml` — CLAUDE.md invariant 5), `test_engineless_doctor_json_reports_the_absent_engine`, `test_engineless_new_fails_closed_at_exit_3` |

| CF-EPIC-14 criterion | Evidence |
| --- | --- |
| Library and CLI Application engine-path E2E coverage remains green and the default engine/`--legacy` Copier split behaves as documented | the `test_installed_engine_archetype_*` and `test_engineless_copier_*` families above, plus the unchanged editable `tests/test_e2e_engine_generation.py` / `tests/test_e2e_generation.py` |
| Failure cases for missing/out-of-range engines, invalid selections, lock failure, and destination conflicts leave no partial project | `test_engineless_new_fails_closed_at_exit_3` (missing engine), `test_out_of_range_engine_is_rejected_before_any_write`, the `test_installed_failure_case_is_rejected_cleanly` matrix, `test_installed_lock_failure_leaves_no_partial_project`, `test_installed_non_empty_destination_is_preserved` |
...
| Case | Status |
| --- | --- |
| unknown `--archetype`; `--archetype` combined with `--legacy`; `--yes` without `--archetype` | `1` |
| unknown `--capability`; wrong-kind `--capability library`; `--capability` + `--no-capabilities`; a selection flag combined with `--legacy` | `1` |
...
| `--template` on the default engine path (requires `--legacy`) | `1` |
| malformed `--data`; malformed `--component-option` | `2` |
| non-empty destination, engine path and `--legacy` Copier path | `1`, contents preserved |
...
This was an E2E test and documentation change only at the time CF-14.03
closed: no shipped module, public API, CLI flag, schema, dependency range,
protocol, component identifier, runtime behaviour, or default path changed,
and `--engine-preview` stayed hidden and opt-in. CF-18.01 later removed that
flag and made the engine path the default — see
[`docs/cli-conventions.md`](cli-conventions.md) for current behaviour; this
record's Environments and failure-matrix sections above are kept in sync with
that change, but this paragraph describes CF-14.03's own boundary as it stood
at the time. CF-14.04
