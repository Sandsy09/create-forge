...
This was an E2E test and documentation change only at the time CF-14.02
closed: it added no component knowledge to a shipped module and changed no
CLI surface, adapter, dependency, protocol, lock, template registry, or
generated byte, and `--engine-preview` stayed hidden and opt-in. CF-18.01
later removed that flag and made the engine path the default — see
[`docs/cli-conventions.md`](cli-conventions.md) for current behaviour. The
installed Library/CLI, default Copier,
