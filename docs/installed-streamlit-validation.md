...
| `uv run poe docs:build` (strict) | passed; `site/streamlit` is absent, so the excluded page is not published |
...
page is written and tested here but **excluded from the published site** (see
`mkdocs.yml`), because published `create-forge 0.4.0` cannot select Streamlit yet.

CF-21.03 ([#167](https://github.com/Sandsy09/create-forge/issues/167)) inherits
the release: removing that exclusion, adding the nav entry and a `projects.md`
archetype row, updating `site_description`, `installation.md` and `reference.md`,
bumping the version, and publishing. Interactive selection at the installed level
